# rusty_rtos_mqtt — the slices

coreMQTT v5.0.2 was remade in twenty-four slices, each diffed against the C
compiled verbatim from the pinned checkout at `04845c6a`. This is the account of
what each slice found: the divergences from MQTT 5.0, the poisons that would not
fire and why, and the defects the differential turned up in the C itself.

Every number, with the command that produced it, is in [`LEDGER.md`](LEDGER.md).
The summary is in the [README](../README.md#conformance).
